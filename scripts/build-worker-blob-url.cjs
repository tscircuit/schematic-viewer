...
const build = async () => {
  try {
    if (!fs.existsSync(path.dirname(workerOutputPath))) {
      fs.mkdirSync(path.dirname(workerOutputPath), { recursive: true });
    }
    const workerCode = fs.readFileSync(workerInputPath, "utf-8");
...
`
    ...
    fs.writeFileSync(workerOutputPath, blobUrlModule);
