...
import { ComponentBuilder, ComponentBuilderClass } from "./ComponentBuilder"
...
export interface ResistorBuilder {
  project_builder: ProjectBuilder
  ports: PortsBuilder
  setName: (name: string) => ResistorBuilder
  setTag: (tag: string) => ResistorBuilder
  setTags: (tags: string[]) => ResistorBuilder
  setSourceProperties(
    properties: Type.SimpleResistor & { name?: string }
  ): ResistorBuilder
  setSchematicCenter(x: number, y: number): ResistorBuilder
  setSchematicRotation(rotation: number | `${number}deg`): ResistorBuilder
  setSchematicProperties(
    properties: Partial<Type.SchematicComponent>
  ): ResistorBuilder
  labelPort(position: number, name: string): ResistorBuilder
